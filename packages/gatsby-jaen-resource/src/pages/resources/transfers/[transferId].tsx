...
// ---- day helpers (local day keys for grouping + today/tomorrow highlighting) ----
const pad2 = (n: number) => String(n).padStart(2, '0')

const toDayKeyLocal = (d: Date) =>
  `${d.getFullYear()}-${pad2(d.getMonth() + 1)}-${pad2(d.getDate())}`

const formatDayKeyLocal = (dayKey: string) => {
  if (!/^\d{4}-\d{2}-\d{2}$/.test(dayKey)) return formatDate(dayKey)
  const [y, m, d] = dayKey.split('-')
  const date = new Date(Number(y), Number(m) - 1, Number(d))
  return date.toLocaleDateString('de-AT')
}

const getTransferDayKey = (t: any) => {
  const raw = t?.rideDateISO || t?.requestedAtISO
  if (!raw) return '—'

  // rideDateISO is commonly already YYYY-MM-DD
  if (
    typeof t?.rideDateISO === 'string' &&
    /^\d{4}-\d{2}-\d{2}/.test(t.rideDateISO)
  ) {
    return t.rideDateISO.slice(0, 10)
  }
...
const Page: React.FC = () => {
  ...
  const [driverColors, setDriverColors] = React.useState<Record<string, string>>(
    {}
  )

  React.useEffect(() => {
    const ids = Array.from(
      new Set(transfers.map((t: any) => t.driverId).filter(Boolean))
    ) as string[]

    const toFetch = ids.filter(id => !!id && !driverColors[id])
    if (!toFetch.length) return

    ;(async () => {
      for (const id of toFetch) {
        try {
          const color = await fetchDriverColor(id)
          setDriverColors(prev => ({...prev, [id]: color ?? 'gray.200'}))
        } catch (err) {
          setDriverColors(prev => ({...prev, [id]: 'gray.200'}))
        }
      }
    })()
  ...
  const todayDayKey = React.useMemo(() => toDayKeyLocal(new Date()), [])
  const tomorrowDayKey = React.useMemo(() => {
    const d = new Date()
    d.setDate(d.getDate() + 1)
    return toDayKeyLocal(d)
  }, [])

  // ONLY indicator on rows/cards: border-right color (green=today, yellow=tomorrow)
  const getRightBorderColor = React.useCallback(
    (dayKey: string) => {
      if (dayKey === todayDayKey) return 'green.400'
      if (dayKey === tomorrowDayKey) return 'yellow.400'
      return undefined
    },
    [todayDayKey, tomorrowDayKey]
  )

  // separator accent: desktop background + text, mobile headline text
  const getSeparatorAccent = React.useCallback(
    (dayKey: string) => {
      if (dayKey === todayDayKey) {
        return {bg: 'green.50', text: 'green.700'}
      }
      if (dayKey === tomorrowDayKey) {
  ...
  const selectedTransfer = React.useMemo(
    () => sortedTransfers.find((t: any) => t.id === selectedTransferId) ?? null,
    ...
    <>
      <Stack spacing={{base: 4, md: 6}}>
        ...
        <Stack spacing={3} display={{base: 'flex', md: 'none'}}>
          ...
          {(() => {
            let lastDayKey = ''
            return sortedTransfers.map((transfer: any, index: number) => {
              const dayKey = getTransferDayKey(transfer)
              const isNewDay = dayKey !== lastDayKey
              lastDayKey = dayKey

              const rightBorderColor = getRightBorderColor(dayKey)
              const sep = getSeparatorAccent(dayKey)
              ...
              const canTerminate =
                ...
                !['terminated', 'canceled', 'complete'].includes(transfer.state)

              const canAssign = isAdminUI && transfer.state !== 'complete'
              ...
              return (
                <React.Fragment key={transfer.id}>
                  ...
                  <Box
                    ...
                    borderLeftWidth="6px"
                    borderLeftColor={
                      driverColors[transfer.driverId] ??
                      transfer.driverColor ??
                      'gray.200'
                    }
                    borderRightWidth={rightBorderColor ? '6px' : undefined}
                ...
                return (
                  <React.Fragment key={transfer.id}>
                      ...
                      <Tr>
                        <Td colSpan={colSpan} p={0}>
                          <HStack
                            ...
                            <Text fontSize="xs" fontWeight="bold" color={sep.text}>
                              {formatDayKeyLocal(dayKey)}
                            </Text>
                          </HStack>
                        </Td>
                    ...
                    <Tr _hover={{bg: 'gray.50'}}>
                      ...
                      <Td>
                        <Text fontSize="sm">{transfer.rideTime || '—'}</Text>
                      </Td>
                      ...
                      <Td>
                        <Text fontSize="sm">{transfer.payment || '—'}</Text>
                      </Td>
                      ...
                      {/* ONLY row indicator for today/tomorrow: border-right in last cell */}
                      <Td
                        textAlign="right"
                        borderRightWidth={rightBorderColor ? '6px' : undefined}
                        borderRightColor={rightBorderColor}>
      ...
      <Modal isOpen={isCompleteOpen} onClose={handleCloseCompleteModal} isCentered>
        <ModalOverlay />
        <ModalContent>
          ...
          <ModalBody>
            <Text fontSize="sm">
              Are you sure you want to mark this transfer as complete? This action
              might not be easily reversible.
            </Text>
          </ModalBody>
          <ModalFooter>
            <Button
      ...
      <Modal isOpen={isTerminateOpen} onClose={handleCloseTerminateModal} isCentered>
        <ModalOverlay />
        <ModalContent>
